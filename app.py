...
def ahora_rd_str() -> str:
    return datetime.now(ZONA_RD).strftime("%H:%M:%S")
...
.ship-desc { font-size: 0.85rem; color: #6B7280; margin-top: 4px; }
.status-badge {
...
.tbl-header {
    display: grid;
    grid-template-columns: 1.15fr 1.5fr 1.2fr 0.6fr 0.9fr 0.95fr 1.35fr;
    padding: 10px 16px;
    font-size: 0.68rem;
    text-transform: uppercase;
...
.tbl-desc { color: #6B7280; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

/* Por defecto (desktop/tablet): tabla visible, tarjetas ocultas */
...
@media (max-width: 640px) {
    ...
    /* KPI buttons más compactos en móvil */
...
def load_data() -> pd.DataFrame:
    ...
    for categoria in CATEGORIAS:
        ...
        # Descarta solo filas realmente vacías (ni BL ni Descripción)
...
def _con_manejo_apierror(func):
    """Decorador: convierte APIError de gspread en (False, mensaje) en vez de tumbar la página."""
...
def cargar_historico_recibidos() -> pd.DataFrame:
    ...
    columnas = ["BL", "Descripcion", "Cantidad", "Fecha_Recibido", "Categoria_Origen"]
    if ws is None:
...
def marcar_como_recibido(bl: str, categoria: str):
    """Devuelve (exito: bool, mensaje_error: str)."""
    ...
    # específico, no el día en que un admin presiona el botón.
    fecha_llegada = None
...
def estado_embarque(eta_str: str):
    ...
    salvo cuando el estado es 'En Puerto' o 'Próximo a llegar'."""
    eta = None
    for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%d-%m-%Y"):
        try:
...
def _prioridad_orden(row) -> tuple:
    """Orden de urgencia: En Puerto (más días primero) → Próximo (menos días primero) → En tránsito → Sin fecha."""
    et = row["EstadoTexto"]
    if et == "En Puerto":
        return (0, -(row["DiasEnPuerto"] or 0))
    if et == "Próximo a llegar":
        return (1, row["DiasEnPuerto"] if pd.notna(row["DiasEnPuerto"]) else 99)
    if et == "En tránsito":
        return (2, str(row.get(COL_ETA) or "9999-99-99"))
    return (3, "")
...
def mostrar_dashboard(df: pd.DataFrame):
    ...
    st.markdown(
        ...
        '<div style="width:52px; height:4px; background:#2E86DE; border-radius:2px; margin:0 auto 0.5rem auto;"></div>'
        '<div style="font-size:1rem; color:#6B7280; font-weight:400;">Antillana Comercial</div>'
        '</div>',
        unsafe_allow_html=True,
    )

    # Indicador de frescura + botón actualizar
    col_info, col_btn = st.columns([4, 1])
    with col_info:
        st.caption(f"Datos con caché de 45 s · Hora RD {ahora_rd_str()} · Usa «Actualizar» si necesitas datos frescos")
    with col_btn:
        if st.button("🔄 Actualizar", key="btn_refresh_global", help="Fuerza recarga desde Google Sheets"):
            load_data.clear()
            contar_recibidas_mes_actual.clear()
            cargar_historico_recibidos.clear()
            st.rerun()
    ...
    hoy = hoy_rd()
    mes_actual_txt = f"{MESES_ES[hoy.month]} {hoy.year}"
    st.markdown(
...
def _render_categoria(df: pd.DataFrame, rol: str, tab_key: str):
        ...
        with col:
            with st.container(key=f"kpi_{tab_key}_{slug}"):
                if st.button(f"{label} — {valor}", key=f"btn_kpi_{tab_key}_{slug}", use_container_width=True):
                    ...
                    # Al filtrar por KPI también limpiamos país para que el filtro sea limpio
    ...
    with g1:
        ...
        if estados_presentes:
            ...
            seleccion_dona = st.plotly_chart(
                fig_dona,
            ...
            puntos_dona = (seleccion_dona or {}).get("selection", {}).get("points", [])
            if puntos_dona:
                label_clic = puntos_dona[0].get("label")
                if label_clic in STATUS_ORDER:
                    firma = f"{tab_key}:dona:{label_clic}"
                    key_firma = f"ultima_firma_dona_{tab_key}"
                    if firma != st.session_state.get(key_firma):
                        st.session_state[f"estado_{tab_key}"] = label_clic
                        st.session_state[f"pais_{tab_key}"] = "Todos"
                        st.session_state[key_firma] = firma
    ...
    # Orden por urgencia real de negocio
    if not filtrado.empty:
        filtrado = filtrado.copy()
        filtrado["_prio"] = filtrado.apply(_prioridad_orden, axis=1)
        filtrado = filtrado.sort_values("_prio").drop(columns=["_prio"])

    # -------------------- LISTA DE EMBARQUES --------------------
    if filtrado.empty:
        st.info("No hay embarques con los filtros seleccionados.")
    else:
        ...
        for _, r in filtrado.iterrows():
            ...
            bl_mostrar = r["BL"] if str(r["BL"]).strip() else "— (sin BL)"
            tabla_html += (
                f'<div class="tbl-row" style="border-left-color:{color};">'
                f'<div class="tbl-bl">{bl_mostrar}</div>'
                f'<div class="tbl-desc" title="{r["Descripcion"] or ""}">{r["Descripcion"] or "—"}</div>'
        ...
        for _, r in filtrado.iterrows():
            ...
            cards_html += (
                ...
                f'<div class="ship-bl">{bl_mostrar}</div>'
    ...
    # -------------------- ACCIONES DE ADMINISTRADOR (dentro de expander) --------------------
    if rol == "admin" and not filtrado.empty:
        with st.expander("Acciones de administrador — marcar recibido / eliminar", expanded=False):
            st.caption("Solo se listan los embarques visibles con el filtro actual.")
            for _, r in filtrado.iterrows():
                bl_actual = r["BL"]
                ...
                if st.session_state.get(key_confirmar):
                    ...
                    st.caption(
                        f"⚠️ \"{r['Descripcion'] or 'Sin descripción'}\" no tiene BL asignado — "
                        "asígnale un BL en el Sheet para poder gestionarlo desde aquí."
                    )
                else:
                    ac0, ac1, ac2, _ = st.columns([1.4, 1.5, 1, 2.1])
                    ac0.markdown(f"**{bl_actual}**")
                ...
                if bl.strip() in bls_existentes:
                    ...
                    if ok:
                        ...
                        st.error(
                            ...
                            "Créala primero y vuelve a intentar."
                        )
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Cerrar sesión"):
            ...
            st.rerun()
        st.divider()
        st.caption("Caché de datos: 45 segundos")
        if st.button("🔄 Forzar actualización", key="sidebar_refresh"):
            load_data.clear()
            contar_recibidas_mes_actual.clear()
            cargar_historico_recibidos.clear()
            st.rerun()
