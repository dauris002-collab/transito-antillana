...
===========================================================
Versión actualizada para subir a GitHub.

Mantiene la estructura original (Streamlit + Google Sheets + categorías +
flujo de 5 etapas + histórico + log + roles), con mejoras funcionales:

- Limpieza dura contra NaN / 'nan'.
- Soporte de fila exacta (FilaSheet) para acciones cuando hay BL duplicados.
- Detección de duplicados por clave funcional (BL + ETA + descripción).
- Confirmación de llegada incluso para embarques marcados como retrasados.
- Botón "Marcar como recibido" solo visible para admin en el panel de proceso.
- Gráficos y contadores más precisos (pendiente vs. en proceso).
- Exportaciones más limpias y útiles.
- Reversa de recibido restaura más trazabilidad.
"""
...
NOMBRES_VALOR = {
    "valor_usd", "valor", "monto", "valor_cif", "monto_usd",
    "valor us$", "valor us"
}
...
ALL_COLUMNS = REQUIRED_COLUMNS + [
    COL_DIAS_PUERTO, COL_ACTUALIZACION, COL_ACTUALIZADO_POR,
...
COLUMNAS_INTERNAS = {
    "Categoria", "FilaSheet", "EstadoTexto", "DiasRel", "ETAFecha",
...
.kpi-card {
    border-radius:14px; padding:14px 18px; min-height:92px; height:100%;
...
.lista {
    border:1px solid var(--ant-borde); border-radius:12px; overflow:hidden;
...
.ficha-k {
    color:#9CA3AF; font-size:0.72rem; text-transform:uppercase; letter-spacing:0.04em;
...
@media print {
    ...
    .badge, .kpi-card { -webkit-print-color-adjust:exact; print-color-adjust:exact; }
}
...
def txt(valor) -> str:
    """Convierte cualquier valor a texto limpio, neutralizando NaN/None/'nan'."""
    if valor is None:
        return ""
...
def _etiquetas_desambiguadas(df: pd.DataFrame, con_categoria: bool = True, largo_desc: int = 38) -> list:
    """Etiquetas para selectbox. Si se repiten, agrega FilaSheet."""
    base = []

    for _, r in df.iterrows():
        bl = txt(r.get(COL_BL, "")) or "(sin BL)"
        desc = txt(r.get(COL_DESC, ""))[:largo_desc] or "sin descripción"
    ...
    return [
        f"{etq} · fila {txt(r.get('FilaSheet', '?'))}" if conteo.get(etq, 0) > 1 else etq
...
def _clave_embarque(row) -> str:
    """Clave funcional para duplicados: BL + ETA + descripción."""
    bl = txt(row.get(COL_BL, "")).casefold()
    desc = txt(row.get(COL_DESC, "")).casefold()

    eta_fecha = parsear_fecha(row.get(COL_ETA, ""))
    eta_txt = eta_fecha.isoformat() if eta_fecha else txt(row.get(COL_ETA, "")).casefold()

    return f"{bl}|{eta_txt}|{desc}"


def _claves_existentes(datos: dict) -> set:
    claves = set()

    for cuadro in (datos.get("activos"), datos.get("historico")):
        if cuadro is None or cuadro.empty:
            continue

        for _, r in cuadro.iterrows():
            claves.add(_clave_embarque(r))
...
def _fila_desde_dict(headers: list, datos: dict) -> list:
    normalizado = {_norm(k): txt(v) for k, v in datos.items()}
...
def _buscar_fila_por_bl(ws, bl: str, fila_esperada=None):
    """Busca la fila por BL. Si viene fila_esperada, la valida primero."""
    if ws is None:
        return None
    ...
    if fila_esperada:
        try:
            fila = int(float(fila_esperada))
        except Exception:
            fila = None

        if fila and fila >= 2:
            try:
                valor = ws.cell(fila, idx + 1).value
                if txt(valor).casefold() == bl.casefold():
                    return fila
            except Exception:
                pass

    try:
        celda = ws.find(bl, in_column=idx + 1)
    except Exception:
        return None
...
_ALIAS_PAISES_CRUDO = {
    "Estados Unidos": [
        "usa", "us", "u.s.a", "u.s.a.", "u.s.", "eeuu", "ee.uu", "ee.uu.", "eu",
...
_VACIOS_PAIS = {
    "", "n/a", "na", "n.a.", "-", "--", "s/d", "nd", "no aplica",
    "pendiente", "?", "nan", "none", "nat"
}


def unificar_paises(serie: pd.Series) -> pd.Series:
    valores = [txt(v) for v in serie]
    grupos = {}
    ...
    canonico = {
        c: max(op.items(), key=lambda kv: (kv[1], -len(kv[0])))[0]
        for c, op in grupos.items()
    }
...
def columnas_extra(df: pd.DataFrame) -> list:
    conocidas = set(ALL_COLUMNS) | COLUMNAS_INTERNAS | {
        "Fecha_Recibido", "Categoria_Origen", "Registrado_Por"
    }
...
def cargar_todo() -> dict:
    ...
    if not historico.empty:
        historico[COL_BL] = historico[COL_BL].map(txt)
        historico = historico[historico[COL_BL] != ""].reset_index(drop=True)
    ...
    if ultima_carga is not None:
        for cuadro in (activos, historico):
            ...
            for marca, autor in zip(cuadro[COL_ACTUALIZACION], cuadro[columna_autor]):
                if parsear_marca(marca) == ultima_carga and txt(autor):
                    ultima_persona = txt(autor)
                    break
...
def append_row(datos: dict, categoria: str):
    ...
    datos = {k: txt(v) for k, v in datos.items()}
    datos[COL_ACTUALIZACION] = marca_ahora()
    datos[COL_ACTUALIZADO_POR] = usuario_actual()

    columnas_a_asegurar = list(dict.fromkeys(
        REQUIRED_COLUMNS + [c for c, v in datos.items() if txt(v) != ""]
    ))
...
def append_rows_bulk(df: pd.DataFrame, categoria: str):
    ...
    def _serie_tiene_valor(col: str) -> bool:
        if col not in df.columns:
            return False
        return df[col].map(txt).ne("").any()

    trae_salida = _serie_tiene_valor(COL_FECHA_SALIDA)
    trae_oc = _serie_tiene_valor(COL_OC)
    trae_ee = _serie_tiene_valor(COL_EE)
    ...
    columnas_fila = (
        REQUIRED_COLUMNS
        + ([COL_FECHA_SALIDA] if trae_salida else [])
        + ([COL_OC] if trae_oc else [])
        + ([COL_EE] if trae_ee else [])
    )

    filas = []

    for _, r in df.iterrows():
        datos = {c: txt(r.get(c, "")) for c in columnas_fila}
...
@_con_manejo_apierror
def actualizar_embarque(bl_original: str, categoria: str, datos: dict, fila_sheet=None):
    ws = get_worksheet(categoria)

    if ws is None:
        return False, f"No existe la pestaña '{categoria}'."

    fila = _buscar_fila_por_bl(ws, bl_original, fila_sheet)
...
@_con_manejo_apierror
def marcar_estatus_llegada(bl: str, categoria: str, valor: str, fila_sheet=None):
    ws = get_worksheet(categoria)

    if ws is None:
        return False, f"No existe la pestaña '{categoria}'."

    fila = _buscar_fila_por_bl(ws, bl, fila_sheet)
    ...
    peticiones = [
        {
            "range": rowcol_to_a1(fila, indices[_norm(COL_ESTATUS_LLEGADA)]),
            "values": [[txt(valor)]],
        },
        {
            "range": rowcol_to_a1(fila, indices[_norm(COL_ACTUALIZACION)]),
            "values": [[marca_ahora()]],
        },
        {
            "range": rowcol_to_a1(fila, indices[_norm(COL_ACTUALIZADO_POR)]),
...
@_con_manejo_apierror
def avanzar_estado_puerto(bl: str, categoria: str, nueva_etapa: str, fila_sheet=None):
    ...
    headers = _asegurar_columnas(
        ws,
        [*columnas_fecha, COL_ESTATUS_LLEGADA, COL_ACTUALIZACION, COL_ACTUALIZADO_POR],
    )
    ...
    if not txt(combinado.get(columna_fecha, "")).strip():
        peticiones.append(
            {
                "range": rowcol_to_a1(fila, indices[_norm(columna_fecha)]),
    ...
    for etapa_posterior in ETAPAS_PUERTO[idx_nueva + 1:]:
        ...
        if txt(combinado.get(col_posterior, "")).strip():
            peticiones.append(
                {
                    "range": rowcol_to_a1(fila, indices[_norm(col_posterior)]),
                    "values": [[""]],
                }
            )
...
@_con_manejo_apierror
def eliminar_embarque(bl: str, categoria: str, fila_sheet=None):
    ws = get_worksheet(categoria)
...
@_con_manejo_apierror
def marcar_como_recibido(bl: str, categoria: str, fila_sheet=None):
    ws_origen = get_worksheet(categoria)

    if ws_origen is None:
        return False, f"No existe la pestaña '{categoria}'."

    fila = _buscar_fila_por_bl(ws_origen, bl, fila_sheet)
    ...
    datos = {h: txt(valores[i]) for i, h in enumerate(headers_origen)}
    datos_norm = {_norm(k): v for k, v in datos.items()}

    faltantes = [
        etapa for etapa in ETAPAS_PUERTO[:-1]
        if not txt(datos_norm.get(_norm(COLUMNA_FECHA_ETAPA[etapa]), "")).strip()
    ]
    ...
    eta_crudo = txt(datos_norm.get(_norm(COL_ETA), ""))
    ...
    registro = {
        COL_BL: txt(datos_norm.get(_norm(COL_BL), bl)),
        COL_DESC: txt(datos_norm.get(_norm(COL_DESC), "")),
        COL_MODELO: txt(datos_norm.get(_norm(COL_MODELO), "")),
        COL_CANT: txt(datos_norm.get(_norm(COL_CANT), "")),
        COL_PAIS: txt(datos_norm.get(_norm(COL_PAIS), "")),
    ...
    columnas_destino = list(COLUMNAS_RECIBIDO)

    for columna_fecha in (
        COL_FECHA_LLEGADA_PUERTO,
        COL_FECHA_DECLARACION,
        COL_FECHA_SOLICITUD_PAGO,
        COL_FECHA_PAGO,
    ):
        valor = txt(datos_norm.get(_norm(columna_fecha), ""))
        if valor:
            registro[columna_fecha] = valor

    fecha_almacen_cruda = txt(datos_norm.get(_norm(COL_FECHA_ALMACEN), ""))
    fecha_almacen = parsear_fecha(fecha_almacen_cruda) or hoy_rd()
    registro[COL_FECHA_ALMACEN] = fecha_almacen.isoformat()

    for col in (COL_FECHA_SALIDA, COL_OC, COL_EE):
        valor = txt(datos_norm.get(_norm(col), ""))
        if valor:
            registro[col] = valor
            columnas_destino.append(col)

    headers_destino = _asegurar_columnas(ws_destino, columnas_destino)

    ws_destino.append_row(
        _fila_desde_dict(headers_destino, registro),
        value_input_option="RAW",
    )
...
@_con_manejo_apierror
def quitar_de_recibido(bl: str, categoria_manual: str | None = None, fila_sheet=None):
    ws_recibido = get_worksheet(RECIBIDO_SHEET)

    if ws_recibido is None:
        return False, f"No existe la pestaña '{RECIBIDO_SHEET}'."

    fila = _buscar_fila_por_bl(ws_recibido, bl, fila_sheet)
    ...
    datos_norm = {_norm(h): txt(valores[i]) for i, h in enumerate(headers)}
    ...
    restaurar = {
        ...
        COL_CANT: txt(datos_norm.get(_norm(COL_CANT), "")),
        COL_PAIS: txt(datos_norm.get(_norm(COL_PAIS), "")),
        COL_ETA: txt(datos_norm.get(_norm(COL_ETA), "")),
    }

    columnas_restaurables = [
        COL_FECHA_SALIDA,
        COL_OC,
        COL_EE,
        COL_FECHA_LLEGADA_PUERTO,
    ...
    for col in columnas_restaurables:
        valor = txt(datos_norm.get(_norm(col), ""))
        if valor:
            restaurar[col] = valor

    ok, mensaje = append_row(restaurar, categoria)
...
def enriquecer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty:
        for c in (
            "EstadoTexto", "DiasRel", "ETAFecha", "Prioridad", "OrdenSec",
            "ValorNum", "DiasTransito", "DiasSolicitudPago",
            "DiasPagoDespacho", "EtapaActual"
        ):
            df[c] = []
    ...
    def _fechas_col(col: str):
        if col not in df.columns:
            return [None] * len(df)
        return [parsear_fecha(v) for v in df[col]]

    salidas = _fechas_col(COL_FECHA_SALIDA)
    llegadas_puerto = _fechas_col(COL_FECHA_LLEGADA_PUERTO)
    declaraciones = _fechas_col(COL_FECHA_DECLARACION)
    solicitudes_pago = _fechas_col(COL_FECHA_SOLICITUD_PAGO)
    pagos = _fechas_col(COL_FECHA_PAGO)
    almacenes = _fechas_col(COL_FECHA_ALMACEN)

    df["EtapaActual"] = [
        _etapa_de_fechas(fechas)
        for fechas in zip(llegadas_puerto, declaraciones, solicitudes_pago, pagos, almacenes)
    ]

    df["DiasTransito"] = [
        max((ll - sal).days, 0) if (sal and ll)
        else (max((hoy - sal).days, 0) if sal else None)
        for sal, ll in zip(salidas, llegadas_puerto)
    ]

    df["DiasSolicitudPago"] = [
        max((pg - sol).days, 0) if (sol and pg)
        else (max((hoy - sol).days, 0) if sol else None)
        for sol, pg in zip(solicitudes_pago, pagos)
    ]

    df["DiasPagoDespacho"] = [
        max((hoy - pg).days, 0) if pg else None
        for pg in pagos
    ]

    def _orden_secuencia(estado: str, dias, fecha):
        if estado in (EST_PUERTO, EST_RETRASADO):
            try:
                return -int(dias) if dias is not None else 0
            except Exception:
                return 0

        return fecha.toordinal() if fecha else 10**9

    df["OrdenSec"] = [
        _orden_secuencia(e, d, f)
        for e, d, f in zip(df["EstadoTexto"], df["DiasRel"], df["ETAFecha"])
    ]
...
def ordenar_vista(df: pd.DataFrame, criterio: str) -> pd.DataFrame:
    ...
    if criterio == "ETA más próximo":
        return df.sort_values("ETAFecha", ascending=True, na_position="last", kind="stable")

    if criterio == "ETA más lejano":
        return df.sort_values("ETAFecha", ascending=False, na_position="last", kind="stable")

    if criterio == "BL":
        return df.sort_values(COL_BL, key=lambda s: s.map(txt).str.casefold(), kind="stable")

    if criterio == "País":
        return df.sort_values([COL_PAIS, "Prioridad"], kind="stable")

    if criterio == "Descripción":
        return df.sort_values(COL_DESC, key=lambda s: s.map(txt).str.casefold(), kind="stable")
    ...
    if ahora < bloqueo:
        ...
        with centro:
            st.error(
                ...
                f"{restante // 60} min {restante % 60} seg."
            )
...
def encabezado(datos: dict):
    ...
    persona = txt(datos.get("ultima_persona", ""))

    if ultima:
        sello = (
            f"Información actualizada: {ultima.day:02d} {MESES_ES_CORTO[ultima.month]} "
...
def render_lista(df: pd.DataFrame):
    if df.empty:
        st.markdown(
            '<div class="lista"><div class="vacio">No hay embarques que coincidan con el filtro.</div></div>',
            unsafe_allow_html=True
        )
        return
    ...
    for _, r in df.iterrows():
        ...
        etapa_puerto = txt(r.get("EtapaActual", ""))
        if etapa_puerto:
        ...
        partes.append(
            ...
            f'<div class="c-bl" data-l="BL">{esc(bl) if bl else "(sin BL)"}</div>'
...
def grafico_linea_tiempo(df: pd.DataFrame, key: str):
    if df.empty:
        st.caption("Sin datos para proyección temporal.")
        return
    ...
    etapa = df.get("EtapaActual", pd.Series([""] * len(df), index=df.index)).map(txt)
    sin_etapa = etapa.eq("")

    retrasados = int(((df["EstadoTexto"] == EST_RETRASADO) & sin_etapa).sum())
    if retrasados:
        etiquetas.append("Retrasados")
        valores.append(retrasados)
        colores.append(STATUS_COLOR[EST_RETRASADO])

    por_confirmar = int(((df["EstadoTexto"] == EST_PUERTO) & sin_etapa).sum())
    if por_confirmar:
        etiquetas.append("Por confirmar")
        valores.append(por_confirmar)
        colores.append(STATUS_COLOR[EST_PUERTO])

    en_proceso = int(((df["EstadoTexto"] == EST_PUERTO) & ~sin_etapa).sum())
    if en_proceso:
        etiquetas.append("En proceso")
        valores.append(en_proceso)
        colores.append(COLOR_TOTAL)

    con_fecha = df[
        df["ETAFecha"].notna()
        & ~df["EstadoTexto"].isin([EST_PUERTO, EST_RETRASADO])
    ]
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                text=[v if v else "" for v in valores],
...
def grafico_paises(df: pd.DataFrame, key: str):
    ...
    fig.update_layout(
        margin=dict(t=18, b=10, l=10, r=24),
        height=260,
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#374151", size=11),
...
def grafico_flujo_puerto(etapa_actual: str, key: str, es_aerea: bool = False):
    ...
    for i in range(n - 1):
        fig.add_trace(go.Scatter(
            ...
            y=[0, 0],
            mode="lines",
            line=dict(color=colores_linea[i], width=5),
            hoverinfo="skip",
            showlegend=False,
        ))
    ...
    anotaciones = [
        dict(
            x=xs[i],
            y=-0.55,
            text=_envolver_etiqueta(etiquetas_mostradas[i]),
            showarrow=False,
            font=dict(size=10, color="#111827" if i == idx_actual else "#6B7280"),
            align="center"
        )
...
def _panel_en_proceso_puerto(df: pd.DataFrame, rol: str, contexto: str):
    ...
    for _, fila in df.sort_values(["Categoria", COL_ETA]).iterrows():
        ...
        etapa_actual = txt(fila.get("EtapaActual", ""))
        fila_sheet = fila.get("FilaSheet")

        clave = (
            f"{_slug_css(contexto)}_{_slug_css(categoria)}_{_slug_css(bl)}_"
            f"{_slug_css(str(fila_sheet or ''))}"
        )

        st.markdown(
            f"**{esc(bl) or '(sin BL)'}** · {esc(fila[COL_DESC])} · {esc(categoria)}"
        )

        grafico_flujo_puerto(
            etapa_actual,
            f"proceso_{clave}",
        ...
        if es_numero(dias_solicitud):
            etiqueta_pago = (
                "tardó en pagarse"
                if txt(fila.get(COL_FECHA_PAGO, "")).strip()
        ...
        if es_admin:
            ...
            if a2.button("Guardar", key=f"guardar_{clave}", width="stretch"):
                ok, mensaje = avanzar_estado_puerto(
                    bl,
                    categoria,
                    nueva_etapa,
                ...
                if ok:
                    accion = (
                        "Avance en puerto"
                        if INDICE_ETAPA.get(nueva_etapa, 0) >= idx_default
            ...
            if st.button(
                ...
                key=f"recibido_{clave}",
                type="primary",
                width="stretch",
            ):
                ok, mensaje = marcar_como_recibido(
                    bl,
                    categoria,
                    fila_sheet=fila_sheet,
                )
            ...
            st.caption("Solo administración puede avanzar etapas o marcar como recibido.")
...
def mostrar_dashboard(datos: dict):
    ...
    seleccion = selector_horizontal(
        ...
        formato=lambda c: etiquetas.get(c, c),
    )
    ...
    if seleccion == "Todos":
        ...
        st.divider()
        _panel_en_proceso_puerto(
            en_proceso_df,
            st.session_state.get("rol", "viewer"),
...
def _render_categoria(df: pd.DataFrame, rol: str, tab_key: str, recibidas_mes: int):
    ...
    etapa_col = df.get("EtapaActual", pd.Series([""] * len(df), index=df.index)).map(txt)
    ...
    valores_serie = df.get("ValorNum", pd.Series([None] * len(df), index=df.index))
    valores = [v for v in valores_serie if es_numero(v)]
    valor_total = sum(valores) if valores else None

    valor_puerto = sum(
        v for v, e, et in zip(valores_serie, df["EstadoTexto"], etapa_col)
        if es_numero(v) and e == EST_PUERTO and et == ""
    ...
    kpis = [
        ...
        (
            f"Recibidas en {MESES_ES[hoy_rd().month]}",
            recibidas_mes,
            COLOR_RECIBIDAS_MES,
            "__historico__",
            "recibidas"
    ...
    if valor_total:
        ...
        v2.markdown(
            tarjeta_kpi(
                "Valor detenido en puerto",
                formato_dinero(valor_puerto or 0),
                "#B45309",
    ...
    paises = ["Todos"] + sorted({p for p in df[COL_PAIS].map(txt) if txt(p)})
    ...
    for k, valid in (
        (f"pais_{tab_key}", paises),
        (f"estado_{tab_key}", estados),
        (f"orden_{tab_key}", criterios),
    ):
    ...
    if busqueda and busqueda.strip():
        ...
        filtrado = filtrado[filtrado.apply(
            lambda r: q in _norm(
                f"{txt(r[COL_BL])} {txt(r[COL_DESC])} {txt(r[COL_MODELO])}"
            ),
            axis=1
        )]
    ...
    if not filtrado.empty:
        with st.expander("Ver ficha completa de un embarque", expanded=False):
            opciones_det = _etiquetas_desambiguadas(
                filtrado,
                con_categoria=False,
                largo_desc=40,
            )
            ...
            elegido = st.selectbox(
                "Embarque",
                opciones_det,
                key=f"detalle_{tab_key}",
                label_visibility="collapsed",
            )

            fila_detalle = filtrado.iloc[opciones_det.index(elegido)]
            _ficha_embarque(fila_detalle)
...
def _panel_confirmacion(df: pd.DataFrame, tab_key: str):
    ...
    tiene_etapa = (
        df.get("EtapaActual", pd.Series([""] * len(df), index=df.index))
        .map(txt)
        .ne("")
    )
    ...
    def _fila_confirmacion(r, viene_retrasado: bool):
        bl = txt(r[COL_BL])
        categoria = r["Categoria"]
        fila_sheet = r.get("FilaSheet")
        ...
        c1.markdown(
            f'<div class="conf-fila">'
            f'<span class="conf-bl">{esc(bl) or "(sin BL)"}</span>'
        ...
        clave = f"{_slug_css(tab_key)}_{_slug_css(bl)}_{_slug_css(str(fila_sheet or ''))}"
        es_aerea_fila = categoria == CATEGORIA_AEREA

        if viene_retrasado:
            etiqueta_boton = (
                "Sí, llegó tarde al aeropuerto"
                if es_aerea_fila
                else "Sí, llegó tarde a puerto"
            )
        else:
            etiqueta_boton = (
                "Sí, llegó al aeropuerto"
                if es_aerea_fila
                else "Sí, llegó a puerto"
            )
        ...
        if c2.button(etiqueta_boton, key=f"si_llego_{clave}", type="primary", width="stretch"):
            ...
            accion_log = (
                "Llegada al aeropuerto confirmada"
                if es_aerea_fila
                else "Llegada a puerto confirmada"
            )

            if viene_retrasado:
                accion_log += " (estaba retrasado)"
        ...
        if viene_retrasado:
            c3.caption("Si sigue retrasado, actualiza el ETA en Editar.")
        else:
            if c3.button("No, está retrasado", key=f"no_llego_{clave}", width="stretch"):
                ok, mensaje = marcar_estatus_llegada(
                    bl,
                    categoria,
                    VALOR_RETRASADO,
                    fila_sheet=fila_sheet,
    ...
    if not pendientes.empty:
        ...
        if len(pendientes) > TOPE:
            st.caption(
                f"…y {len(pendientes) - TOPE} más. Filtra por estado 'En Puerto' para verlos todos."
            )

    if not retrasados.empty:
        st.markdown(
            f'<div class="conf-titulo" style="margin-top:14px;">'
            f'Ya verificados como retrasados · {len(retrasados)}'
            f"</div>",
            unsafe_allow_html=True,
        )
...
def tabla_exportable(df: pd.DataFrame) -> pd.DataFrame:
    ...
    salida = pd.DataFrame({
        "BL": df[COL_BL].map(txt),
        "Descripción": df[COL_DESC].map(txt),
        "Modelo/Serie": df[COL_MODELO].map(txt),
        "Cantidad": df[COL_CANT].map(txt),
        "País de origen": df[COL_PAIS].map(txt),
        "ETA": [formato_eta(v) for v in df[COL_ETA]],
        "Estado": [texto_estado(e, d) for e, d in zip(df["EstadoTexto"], df["DiasRel"])],
        "Categoría": df["Categoria"].map(txt),
    })

    if "EtapaActual" in df.columns:
        salida["Etapa actual"] = df["EtapaActual"].map(txt)

    fechas_flujo = [
        (COL_FECHA_SALIDA, "Fecha salida"),
        (COL_FECHA_LLEGADA_PUERTO, "Llegada puerto/aeropuerto"),
        (COL_FECHA_DECLARACION, "Recepción y declaración"),
        (COL_FECHA_SOLICITUD_PAGO, "Solicitud de pago"),
        (COL_FECHA_PAGO, "Pago realizado"),
        (COL_FECHA_ALMACEN, "Recibido en almacén"),
    ]

    for col, etiqueta in fechas_flujo:
        if col in df.columns:
            salida[etiqueta] = [formato_eta(v) for v in df[col]]

    if "DiasTransito" in df.columns:
        salida["Días tránsito"] = df["DiasTransito"].map(
            lambda v: int(v) if es_numero(v) else ""
        )

    if "DiasSolicitudPago" in df.columns:
        salida["Días solicitud-pago"] = df["DiasSolicitudPago"].map(
            lambda v: int(v) if es_numero(v) else ""
        )

    if "DiasPagoDespacho" in df.columns:
        salida["Días esperando despacho"] = df["DiasPagoDespacho"].map(
            lambda v: int(v) if es_numero(v) else ""
        )

    for extra in columnas_extra(df):
        salida[extra] = df[extra].map(txt)

    if COL_ACTUALIZACION in df.columns:
        salida["Última actualización"] = df[COL_ACTUALIZACION].map(txt)

    if COL_ACTUALIZADO_POR in df.columns:
        salida["Actualizado por"] = df[COL_ACTUALIZADO_POR].map(txt)
...
def _ficha_embarque(fila):
    ...
    if fila["Categoria"] in CATEGORIAS_CON_OC_EE:
        if txt(fila.get(COL_OC, "")):
            campos.append(("OC", fila[COL_OC]))

        if txt(fila.get(COL_EE, "")):
            campos.append(("EE", fila[COL_EE]))

    if txt(fila.get(COL_FECHA_SALIDA, "")):
    ...
    if es_numero(dias_transito):
        ...
        etiqueta_transito = (
            "Tardó en tránsito"
            if txt(fila.get(COL_FECHA_LLEGADA_PUERTO, "")).strip()
    ...
    etapa_puerto = txt(fila.get("EtapaActual", ""))

    if es_maritimo and etapa_puerto:
        etapa_mostrada = (
            "Llegada al aeropuerto"
            if (es_aerea_ficha and etapa_puerto == ETAPAS_PUERTO[0])
            else etapa_puerto
        )

        campos.append((
            "Etapa en aeropuerto" if es_aerea_ficha else "Etapa en puerto",
            etapa_mostrada
        ))

    if txt(fila.get(COL_FECHA_LLEGADA_PUERTO, "")):
        campos.append((
            "Llegada al aeropuerto" if es_aerea_ficha else "Llegada a puerto",
            formato_eta(fila[COL_FECHA_LLEGADA_PUERTO])
        ))

    if txt(fila.get(COL_FECHA_DECLARACION, "")):
        campos.append(("Recepción y declaración", formato_eta(fila[COL_FECHA_DECLARACION])))

    if txt(fila.get(COL_FECHA_SOLICITUD_PAGO, "")):
    ...
    if txt(fila.get(COL_FECHA_PAGO, "")):
    ...
    if txt(fila.get(COL_FECHA_ALMACEN, "")):
    ...
    if txt(fila.get(COL_ACTUALIZACION, "")):
        autor = txt(fila.get(COL_ACTUALIZADO_POR, ""))
    ...
    if es_maritimo and etapa_puerto:
        ...
        clave = (
            f"{_slug_css(str(fila['Categoria']))}_{_slug_css(str(fila[COL_BL]))}_"
...
def _panel_acciones(df: pd.DataFrame, tab_key: str):
    con_bl = df[df[COL_BL].map(txt).str.strip() != ""]
    sin_bl = df[df[COL_BL].map(txt).str.strip() == ""]

    if not sin_bl.empty:
        nombres = ", ".join(txt(d)[:40] for d in sin_bl[COL_DESC].head(5))
        st.caption(
            f"{len(sin_bl)} fila(s) sin BL asignado no se pueden gestionar desde aquí ({nombres})."
        )
    ...
    if es_maritimo_sel and (etapa_actual or fila["EstadoTexto"] in (EST_PUERTO, EST_RETRASADO)):
        ...
        idx_default = INDICE_ETAPA.get(etapa_actual, 0)
        clave_bl = f"{_slug_css(tab_key)}_{_slug_css(bl)}_{_slug_css(str(fila_sheet or ''))}"
        ...
        st.caption(
            f"Llegada sin confirmar todavía — usa {texto_boton_pendiente} arriba, "
    ...
    if c2.button("Editar", key=f"edit_{tab_key}", width="stretch"):
        ...
        st.session_state["editar_fila"] = fila_sheet
        st.session_state["seccion"] = "Editar"
        st.rerun()

    if c3.button("Eliminar", key=f"del_{tab_key}", width="stretch"):
        st.session_state[f"confirmar_del_{tab_key}"] = (bl, categoria, fila_sheet)
        rerun_fragmento()
    ...
    if pendiente:
        ...
        st.warning(
            f"¿Eliminar definitivamente el BL {bl_pend}? No se puede deshacer. "
        ...
        if d1.button("Sí, eliminar", key=f"si_del_{tab_key}", type="primary"):
            ok, mensaje = eliminar_embarque(
                bl_pend,
                cat_pend,
                fila_sheet=fila_pend,
            )
...
def form_alta_manual(datos: dict):
    ...
    if not txt(bl) or not txt(descripcion):
        st.error("BL y Descripción son obligatorios.")
        return

    registro_previo = {
        COL_BL: txt(bl),
        COL_DESC: txt(descripcion),
        COL_ETA: eta.isoformat(),
    }

    if _clave_embarque(registro_previo) in _claves_existentes(datos):
        st.error(
            "Ya existe un embarque con el mismo BL, ETA y descripción "
            "(activo o en el histórico). Si es un embarque distinto, cambia "
            "descripción o ETA."
        )
        return
    ...
    datos_nuevos = {
        COL_BL: txt(bl),
        COL_DESC: txt(descripcion),
        COL_MODELO: txt(modelo),
        COL_CANT: txt(cantidad),
        COL_PAIS: txt(pais),
        COL_ETA: eta.isoformat(),
    }
    ...
    if con_oc_ee:
        if txt(oc):
            datos_nuevos[COL_OC] = txt(oc)

        if txt(ee):
            datos_nuevos[COL_EE] = txt(ee)

    ok, mensaje = append_row(datos_nuevos, categoria)

    if ok:
        registrar_log("Alta manual", txt(bl), categoria, f"ETA {eta.isoformat()}")
        invalidar_caches()
        st.success(f"Embarque {txt(bl)} guardado en '{categoria}'.")
        st.rerun()
    else:
        st.error(mensaje)
...
def form_editar(datos: dict):
    ...
    con_bl = df[df[COL_BL].map(txt).str.strip() != ""].reset_index(drop=True)
    ...
    preseleccion_bl = st.session_state.pop("editar_bl", None)
    preseleccion_fila = st.session_state.pop("editar_fila", None)

    if preseleccion_fila is not None:
        matches = con_bl.index[con_bl["FilaSheet"] == preseleccion_fila]

        if len(matches):
            indice_default = int(matches[0])

    elif preseleccion_bl:
        matches = con_bl.index[con_bl[COL_BL].map(txt) == txt(preseleccion_bl)]
    ...
    with st.form("form_editar"):
        ...
        descripcion = c1.text_input("Descripción", value=txt(fila[COL_DESC]))
        modelo = c2.text_input("Modelo o serie", value=txt(fila[COL_MODELO]))

        c3, c4 = st.columns(2)

        cantidad = c3.text_input("Cantidad", value=txt(fila[COL_CANT]))
        pais = c4.text_input("País de origen", value=txt(fila[COL_PAIS]))
        ...
        if con_oc_ee:
            c7, c8 = st.columns(2)
            oc = c7.text_input("OC", value=txt(fila.get(COL_OC, "")))
            ee = c8.text_input("EE", value=txt(fila.get(COL_EE, "")))

        st.caption(f"ETA actual en el Sheet: {txt(fila[COL_ETA]) or '(vacío)'}")
    ...
    if con_oc_ee:
        cambios[COL_OC] = txt(oc)
        cambios[COL_EE] = txt(ee)
    ...
    if ok:
        ...
        if txt(fila[COL_ETA]) != eta.isoformat():
            detalles.append(f"ETA {txt(fila[COL_ETA]) or 'vacío'} → {eta.isoformat()}")

        registrar_log(
            "Edición",
            txt(fila[COL_BL]),
            categoria,
            "; ".join(detalles) or "campos varios"
        )
...
def form_carga_masiva(datos: dict):
    ...
    nuevo = nuevo[columnas_a_usar].copy()

    for c in nuevo.columns:
        nuevo[c] = nuevo[c].map(txt)

    nuevo = nuevo.dropna(how="all").copy()
    nuevo = nuevo[nuevo[COL_BL].ne("") | nuevo[COL_DESC].ne("")]
    ...
    if COL_FECHA_SALIDA in nuevo.columns:
        nuevo[COL_FECHA_SALIDA] = [
            parsear_fecha(v).isoformat() if parsear_fecha(v) else ""
            for v in nuevo[COL_FECHA_SALIDA]
        ]

    claves = nuevo.apply(_clave_embarque, axis=1)

    dup_archivo = claves.duplicated(keep="first") & claves.ne("")

    if dup_archivo.any():
        st.warning(
            "El archivo trae registros repetidos internamente; solo se cargará la primera aparición de: " +
            ", ".join(sorted(set(nuevo.loc[dup_archivo, COL_BL])))
        )

    existentes = _claves_existentes(datos)
    es_dup = claves.isin(existentes) | dup_archivo

    if es_dup.any():
        ya = sorted({
            b for b in nuevo.loc[claves.isin(existentes), COL_BL]
            if txt(b)
        })

        if ya:
            st.warning(
                "Estos registros ya existen en el sistema (mismo BL, ETA y descripción) "
                "y no se cargarán de nuevo: " + ", ".join(ya)
            )
...
def mostrar_historico(datos: dict, rol: str):
    ...
    k1.markdown(
        tarjeta_kpi(
            f"{MESES_ES[mes_ant[1]]} {mes_ant[0]} · mes anterior",
            n_anterior,
            "#6B7280"
        ),
        unsafe_allow_html=True,
    ...
    k2.markdown(
        tarjeta_kpi(
            f"{MESES_ES[hoy.month]} {hoy.year} · mes en curso",
            n_actual,
            COLOR_RECIBIDAS_MES,
    ...
    k3.markdown(
        tarjeta_kpi(
            f"Acumulado {hoy.year}",
            n_anio,
            COLOR_TOTAL,
            f"{len(df)} en todo el histórico"
        ),
        unsafe_allow_html=True,
    )
    ...
    idx_mes = (
        meses_con_datos.index(hoy.month)
        if (anio_sel == hoy.year and hoy.month in meses_con_datos)
        else 0
    )
    ...
    if busqueda and busqueda.strip():
        ...
        filtrado = filtrado[filtrado.apply(
            lambda r: q in _norm(
                f"{txt(r.get(COL_BL, ''))} {txt(r.get(COL_DESC, ''))} {txt(r.get(COL_MODELO, ''))}"
            ),
            axis=1
        )]
    ...
    with st.expander("Revertir una recepción", expanded=False):
        ...
        guardada = txt(fila.get("Categoria_Origen", ""))
...
def _tabla_detalle(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame(columns=[
            "BL", "Descripción", "Modelo/Serie", "Cantidad",
            "Origen", "Fecha recibido", "Categoría", "Registrado por"
        ])

    salida = pd.DataFrame({
        "BL": df.get(COL_BL, "").map(txt),
        "Descripción": df.get(COL_DESC, "").map(txt),
        "Modelo/Serie": df.get(COL_MODELO, "").map(txt),
        "Cantidad": df.get(COL_CANT, "").map(txt),
        "Origen": df.get(COL_PAIS, "").map(txt),
        "Fecha recibido": [
            f"{f.day:02d} {MESES_ES_CORTO[f.month]} {f.year}"
            for f in df["FechaParsed"]
        ],
        "Categoría": df.get("Categoria_Origen", "").map(txt),
        "Registrado por": df.get("Registrado_Por", "").map(txt),
    })
...
def herramientas(datos: dict):
    ...
    for _, r in df.iterrows():
        ...
        pendientes.append({
            ...
            "bl": txt(r[COL_BL]) or "(sin BL)",
            "descripcion": txt(r[COL_DESC])[:40],
            **diag,
        })
    ...
    if ilegibles:
        st.error(
            ...
            "en el Sheet: " + ", ".join(
                f"{p['bl']} (fila {p['fila']} de {p['categoria']}: '{p['crudo']}')"
                for p in ilegibles
            )
        )
    ...
    if convertibles:
        ...
        st.dataframe(
            pd.DataFrame([{
                ...
                "Fila": p["fila"],
                "Valor actual": p["crudo"],
                "Se guardará como": p["dm"].isoformat(),
    ...
    cambios = [
        (p["categoria"], p["fila"], (p["dm"] if usar_dm else p["md"]).isoformat())
        for p in ambiguas
    ]

    cambios += [
        (p["categoria"], p["fila"], p["dm"].isoformat())
        for p in convertibles
    ]
    ...
    if cambios and st.button(f"Convertir {len(cambios)} fecha(s) a AAAA-MM-DD", type="primary"):
        ...
        if ok:
            registrar_log(
                ...
                "",
                "",
                f"{len(cambios)} celda(s); criterio={criterio}"
            )
            invalidar_caches()
            st.success(mensaje)
            st.rerun()
...
def main():
    ...
    secciones = (
        ["Dashboard", "Agregar", "Editar", "Carga masiva", "Histórico", "Herramientas"]
        if es_admin
        else ["Dashboard", "Histórico"]
    )
